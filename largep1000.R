...
#--------------------------------------------
# state: state of the chain, list containing
#
# list(
#.     #data
#.     n=n, #n. of units, scalar
#      p=p, # n. of observed variables per unit
#      S=S, #n. of groups
#      ns=ns, #n. of units per group, vector of length S 
...
#.     global model selection parameters for the CUSP
...
Gibbs_Kernel=function(state){
  ...
  if(is.null(state$scale_beta)) state$scale_beta=0.2
  if(is.null(state$X)) state$X=matrix(1, ncol=state$d, nrow = state$n)
  ...
  # 9. update ps (local activation parameters)
  mu_eta = tcrossprod(state$eta,state$Lambda)
  pred =  state$X%*%(state$betas)
  logit_phi = plogis(pred)
  mu_phi= tcrossprod(state$phi,state$Gamma)
  ...
  for(h in 1:state$k){
    if(state$tau_phi[h]==0){ state$ps[,h]=0}
  }
  
  w0=which(apply(state$ps,2, function(x) all(x==0)))
  state$tau_phi[w0]=0
  # reorder
  if(length(w0)>0){
  state$tau_phi=c(state$tau_phi[-w0],state$tau_phi[w0])
  state$ps=cbind(state$ps[,-w0],state$ps[,w0])
  state$phi=cbind(state$phi[,-w0],state$phi[,w0])
  state$Gamma=cbind(state$Gamma[,-w0],state$Gamma[,w0])
  }
  return(state)
}
