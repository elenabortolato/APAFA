...
standardize=TRUE
#CHANGE to see results without standardizing
#standardize=FALSE
...
######################################################
# data generating process (scenario A)
setting=1
...
state=list(y=y,# response
             ...
             alpha_eta=3, alpha_phi=3, # equal tO expected n.of active factors+1
             betas=betas, 
...
# convergence assessment: Loglikelihood
iter=1
Loglik=rep(0, 10000)
for(iter in 1:10000){
...
iter=8001
VARR=VARRTRUE=matrix(0,2000,100)
...
boxplot(VARR[,or]/20, outline=T, ylab=expression(lambda[ij]+sigma[ii]))
points(1:100,VARTRUE[or], col=2, cex=0.3)
...
for(iter in 8001:10000){
  ...
  L=t(t(L)*ris_tau_eta[iter,1:6])
  LLT=L%*%t(L)
  #note, i am reordering the specific factors
  G=cbind(ris_gamma1[iter,],ris_gamma3[iter,],ris_gamma2[iter,],
          ris_gamma4[iter,],ris_gamma5[iter,],ris_gamma6[iter,])
  G=t(t(G)*ris_tau_phi[iter,1:6])
  PS=(cbind(ris_ps1[iter,],ris_ps3[iter,],ris_ps2[iter,],
            ris_ps4[iter,],ris_ps5[iter,],ris_ps6[iter,])) 
    S=diag(ris_sigma1[iter,])
   
  
  for(ii in 1:20){
    psi=diag(c(0,0,0,0,0,0))
    psi[trunc((ii+19)/20),trunc((ii+19)/20)]=1
    GGT=  G%*%diag(PS[ii,1:6])%*% t(G)
    
    VAR=GGT
    VARTRUE=  copy_state$Gamma%*%(psi)%*%t(copy_state$Gamma) 
    VARR[iter_, ]= VARR[iter_,  ]+c(VAR)
    VARRTRUE[iter_, ]= VARTRUE+c(VARTRUE)
    }
  print(iter)
}
par(mfrow=c(3,1)) 
par(mar=c(1,4,2,2))
or=order(colMeans(VARR))
boxplot(VARR[,or]/20, outline=F, ylab=expression(gamma[ij]))
points(1:100,colMeans(VARRTRUE[,] )[or], col=2)
