                   ...
                   nrun = NULL, burn = NULL, control = list(...), ...)
{
  ...
  ## Initialize output
  Lambdaout <- Aout <- y_sim <- list()
  Psiout <- l_out <- lapply(1:S, function(x) list())
  y=NULL
  for(s in 1:4) y=rbind(y, Y_s[[s]])
  ## Start posterior sampling
   
  for(iter in 1:nrun){
  
    
  conditional_d<- function (ii,jj) {
    group=which(ii<cumsum(ns))[1]
    
    active=which(A[group,]==1)
    VAR=tcrossprod(t(t(Lambda[,active])))+diag(psi_s[[group]])
    V11=VAR[jj,jj]
    V22=VAR[-jj,-jj]
    V12=VAR[jj,-jj]
    V21=VAR[-jj,jj]
    ...
    Mc=mean(y[,jj])+ V12%*%solve(V22)%*%(y[ii,-jj]-
                                              apply(y,2,mean)[-jj])
    return(rnorm(1,c(Mc), c(Vc)))
  } 
    y_na=NULL
    for(s in 1:4) y_na=rbind(y_na, Y_s[[s]])
    for(i in 1:30){
      ii=idx_imp[i,1]
      jj=idx_imp[i,2]
    ...
    Y_s=list(y_na[1:ns[1],],
             y_na[(ns[1]+1):(ns[1]+ns[2]),],
             y_na[(ns[1]+ns[2]+1):(ns[1]+ns[2]+ns[3]),],
             y_na[(ns[1]+ns[2]+ns[3]+1):sum(ns),])
  ...
  ## Save and exit
  out <- list(Lambda = Lambdaout, Psi = Psiout, l_s = l_out, 
              A = Aout  , y_sim=y_sim, y=y)
                              ...
                              control = list(...), total_iters = NULL, total_burn=NULL, ...)
{
  ...
  ## Save and exit
  out <- list(Lambda = Lambdaout, Psi = Psiout, l_s = l_out, 
              A = Aout, idents = identsout, y_sim=y_sim)
  return(structure(out,  class="sp_msfa"))
}
...
y=readRDS(file="data_immune/immune_data.RDS")
n=nrow(y) # total units
...
X_s=list(y[1:ns[1],],
         y[(ns[1]+1):(ns[1]+ns[2]),],
         y[(ns[1]+ns[2]+1):(ns[1]+ns[2]+ns[3]),],
         y[(ns[1]+ns[2]+ns[3]+1):sum(ns),])

sum(unlist(lapply(X_s, function (x) dim(x)[1])))
...
d=30
ncol(y)
Lambda=matrix(fact_est$loadings[,1:d], ncol = d, nrow = 63)
Lambda
dim(Lambda)
init=list(Lambda=Lambda,
            l_s <- lapply(1:S, function(x) mvrnorm(n_s[x], rep(0, K), diag(rep(1, K))))
)
set.seed(11)
control$burn=0
...
new_control <- function(nrun = 2000, burn = 0, thin = 1,
                        nu = 3, 
                        a1 = 2.1, b1 = 1,
                        a2 = 2.1, b2 = 1,
                        apsi = 1, bpsi = 0.3,
...
control
#11.14
tetris_ris88 <- tetris(X_s,  alpha=4.5, beta=1, trace = TRUE, nprint = 10, fixed = F,control = control)
tetris_ris=tetris_ris88
VAR=tetris_ris$Lambda[[1]]%*%t(tetris_ris$Lambda[[1]])+diag(tetris_ris$Psi[[1]][[1]])
VAR
which(tetris_ris$A[[1]])
VAR=c(tetris_ris$Lambda[[1]]%*%t(tetris_ris$Lambda[[1]])+1/4*(tetris_ris$Psi[[1]][[1]]+tetris_ris$Psi[[2]][[1]]+tetris_ris$Psi[[3]][[1]]+tetris_ris$Psi[[4]][[1]]))

#tetris_risold=tetris_ris
#tetris_ris=tetris_ris88
tetris_ris=tetris_ris88
ITER
L=tetris_ris88$Lambda
L
A=tetris_ris$A[[ITER]]
cols=apply(A,2, function (c) (all(c==1)))
cols=which(cols)
VAR=c(L[,cols]%*%t(L[,cols])+1/4*diag(tetris_ris$Psi[[1]][[ITER]]+
                                        tetris_ris$Psi[[2]][[ITER]]+
                                        tetris_ris$Psi[[3]][[ITER]]+
                                        tetris_ris$Psi[[4]][[ITER]]))

for(ITER in c(2: 200)){
  L=tetris_ris$Lambda[[ITER]]
  ...
  VAR2=c(L[,cols]%*%t(L[,cols])+
           1/4*diag(tetris_ris$Psi[[1]][[ITER]]+
                      tetris_ris$Psi[[2]][[ITER]]+
                      tetris_ris$Psi[[3]][[ITER]]+
                      tetris_ris$Psi[[4]][[ITER]]))
...
par(mfrow=c(1,1))
mem.maxVSize(999999)
orr=order(apply(VAR,2,mean))
boxplot(VAR[,orr], outline=F, ylab=expression(Lambda*Lambda[ij]^T+sigma[ii]))

ITER=1
L=tetris_ris$Lambda[[ITER]]
A=tetris_ris$A[[ITER]]
cols=apply(A,2, function (c) (all(c==1)))
VAR=c(L[,-cols]%*%t(L[,-cols]))

for(ITER in c(2: 2000)){
  L=tetris_ris$Lambda[[ITER]]
...
for(ITER in c(1: 2000)){
  A=tetris_ris$A[[ITER]]
  cols=apply(A,2, function (c) !(all(c==1)))
  Nspec=c(Nspec,(sum(cols)))
  
}
summary(Nspec)
par(mar=c(4,4,4,4))
orr=order(apply(VAR,2,mean))
boxplot(VAR[,orr], outline=F, ylab=expression(Gamma*Gamma[ij]^T))

save.image("tetris_immune_imp.RData")

# Results:

#load(("tetris_immune_imp.RData"))
nfactor=rep(0,2000)
nfactor[1]=ncol(tetris_ris$A[[1]])
for(iter in c(2: 2000)){
  nfactor[iter]=ncol(tetris_ris$A[[iter]])
}
barplot(table(nfactor))
quantile(nfactor,0.05)
quantile(nfactor,0.95)
...
iter=1
for(iter in 1:2000) {
  y_matrix=tetris_ris88$y_sim[[iter]]
  MSE[iter]=  mean(((y_matrix)-(tetris_ris$y/1.22))^2) 
}
...
for(iter in 1:20){
  for(ii in 1:30) diff[iter,ii]=(y[idx_imp[ii,1],idx_imp[ii,2]]/1.22-
                                        tetris_ris88$y_sim[[iter]][idx_imp[ii,1],idx_imp[ii,2]]) 
}
boxplot((diff[1:iter,]))

 
pred=matrix(0, 20,30)
str(tetris_ris88$y_sim)
for(iter in 1:20){
  for(ii in 1:30) pred[iter,ii]=(tetris_ris88$y_sim[[iter]][idx_imp[ii,1],idx_imp[ii,2]]) 
}
yobs=matrix(0,1,30)
boxplot((pred[,])/1.22, outline=F, ylab="prediction")
