...
Gibbs_Kernel=function(state){
  ...
  if(is.null(state$scale_beta)) state$scale_beta=0.01
...
state=list(y=y,# response
             ...
             betas=betas, 
             ps=matrix(rbinom(n*k,1,0.950), nrow=n)
  )
...
print(time)
save.image(paste(setting,"largep1000.RData",sep = ""))
}
