# this file is an example to compute performance metrics 
# for the simulation studies of Section 3 of the paper.

# select the folder where all the results from scenario 1 will be stored
# setwd("Simulations/SCEN1")

plot=0
par(mfrow=c(1,1))

# for preparing the activation pattern mage
partition=matrix(0, ncol=3, nrow=45)
for(setting in 1:10){ #load all the simulations for the first setting
  name=paste(paste("scen1_",setting, sep = ""), ".RData", sep = "")
  load(name)
  MAP<- function(val){
  ...
  # mean of active factors 
  NFACTOR=cbind(cbind(mean(apply(ris_tau_eta,1, sum))),
                cbind(mean(apply(ris_tau_phi,1, sum))))
   
  #trace function
  tr<-function(A) sum(diag(A))
  
  nanzero<-function(a,b=0){
    if(is.nan(b)) return(a)
    else return(b)
  }
  ...
  #order columns of Theta and Psi
  ...
  #for each true specific factor find the distance with estimated columns
  tab=sapply(1:length(copy_state$tau_phi), (function (x) sapply(1:ncol(psest), function (k) (sum(abs(copy_state$ps[,x]-psest[,k]))))))
  
  
  #avoid comparison with non active factors
  tab[,copy_state$tau_phi==0]=copy_state$n*2  #true factor=0
  tab[tau_ph==0,]=copy_state$n*2 #estimated factor=0
  
  #identify the factor with best match
  ordine=order(apply(tab, 2,min))
  
  #factors already matched
  taken=NA
  #index of estimated factors linked to true factors
  ...
  for(i in 1:ncol(tab)){
    ...
    which=1
    while(!found){
      #start matching with best match
    similar_ps[ordine[i]]=order(tab[,ordine[i]])[which]
    if(similar_ps[[ordine[i]]]%in%taken){ which=which+1}
    else {taken=c(taken, similar_ps[ordine[i]]);found=T}
    }
  }
  #new order for the specific factors
  ...
  #true covariance matrix components
  ...
  #performance measure: compute RV coefficient
  gamphi=lapply(1:n, function(i) Gam%*%diag(c(ps[i,]))%*%t(Gam))
  
  lamet=lapply(1:n, function(i) Lam%*%diag(c(th[i,]))%*%t(Lam))
  ...
  norm1=mean(sapply(1:10,function(i) tr(t(lamet[[i]]+gamphi[[i]] +Sig1)%*%(true1) )/
                              sqrt (tr(t(lamet[[i]]+ gamphi[[i]]+Sig1)%*%
                                         (lamet[[i]]+ gamphi[[i]] +Sig1))%*%tr(t(true1)%*%true1))))
    
  norm2=mean(sapply(11:20,function(i) tr(t( lamet[[i]]+ gamphi[[i]]+Sig2)%*%(true2) )/
                              sqrt (tr(t(lamet[[i]]+ gamphi[[i]]+Sig2)%*%
                                         (lamet[[i]]+gamphi[[i]]+Sig2))%*%tr(t(true2)%*%true2))))
    
  norm3=mean(sapply(21:30,function(i) tr(t( lamet[[i]]+gamphi[[i]]+Sig3)%*%(true3))/
                              sqrt (tr(t(lamet[[i]]+ gamphi[[i]]+Sig3)%*%
                                         (lamet[[i]]+ gamphi[[i]]+Sig3))%*%tr(t(true3)%*%true3))))
