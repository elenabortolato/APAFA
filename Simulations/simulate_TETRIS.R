...
if (any(!installed)) {
  install.packages(required_packages[!installed], dependencies = TRUE)
}
...
setwd("Simulatons/long")

# Set parameters and prior hyperparameters for posterior sampling
...
for (setting in 1:10){
  ...
  if(1==1){
    ...
    ks=rep(NA, S)
    ks=c(1,1,2)#specific factors study  
    k=sum(ks)
    group=rep(NA,n)
    for (s in 1:S) {
    ...
    cc=(c(8,-8,-7,-6))
    cc=rep(cc,k*S)
    #betas=(matrix(cc, ncol=k, nrow=S, byrow = F))
    #extra columns
    #betas=betas
    betas=matrix(0, ncol=k, nrow=S)
    for (l in 1:S) {
      whichgroup[[l]]
      betas[l, c(whichgroup[[l]])]=1
    }
    betas 
    #extra columns
    
    betas=betas 
    #betas=0.63*(1/(1:1)*(betas))
    (plogis(betas ))
  ...
  state=list(y=y,# response
             ...
             #prior
             a_lambda=1, b_lambda=1, a_gamma=1, b_gamma=1,
             a_sigma=30, b_sigma=3,  
             alpha_eta=2, alpha_phi=3, # equal tO expected n.of active factors+1
             betas=betas, 
             ps=(cbind(X,X,X,X)[,1:k])*matrix(rbinom(n*k,1,0.5), nrow=n)
             # ps_=matrix(rbinom(n*k,1,0.95), nrow=n)*(cbind(X,X,X,X)[,1:k]),
             #ps__=matrix(rbinom(n*k,1,0.95), nrow=n)*(cbind(X,X,X,X)[,1:k])
  )
 


 copy_state=state
  data_y=list(y[1:20,], y[21:40,], y[41:60,])
  # input: a list of matrices
  # alpha, beta are hyperparameters 
  run <- tetris(data_y,alpha=6,beta=3,trace = T, control=new_control)
  ...
  name=paste(paste("scen1TETRIS1_",setting, sep = ""), ".RData", sep = "")
  name
